...
// ─────────────────────────────────────────────────────────────────────────────
// CONFIGURAÇÕES GLOBAIS E CONSTANTES DO SISTEMA (ATUALIZADO MARÇO/2026)
// ─────────────────────────────────────────────────────────────────────────────
...
// Status para eficiência (mantido – usa coluna SITUACIONAL)
...
const BrazilMap = ({ 
  ...
  const colorScale = scaleLinear().domain([0, maxValue || 1]).range(['#e0f2fe', '#1e40af']);

  return (
    <div ref={containerRef} className="w-full h-full relative overflow-hidden bg-gradient-to-br from-slate-50 to-white">
      <svg ref={svgRef} width={dimensions.width} height={dimensions.height} viewBox={`0 0 ${dimensions.width} ${dimensions.height}`} className="w-full h-full">
        <g>
          {features.map((feature, index) => {
            ...
            const fillColor = isSelected ? '#1d4ed8' : isHovered ? '#60a5fa' : count > 0 ? colorScale(count) : '#f1f5f9';
            ...
            return (
              <path
                ...
                onMouseEnter={(e) => {
                  ...
                  if (rect) setTooltip({ x: e.clientX - rect.left + 20, y: e.clientY - rect.top + 20, sigla, count });
                }}
                onMouseMove={(e) => {
                  const rect = svgRef.current?.getBoundingClientRect();
                  if (rect && tooltip) setTooltip(prev => ({ ...prev, x: e.clientX - rect.left + 20, y: e.clientY - rect.top + 20 }));
                }}
      ...
      {tooltip && (
        <div className="absolute pointer-events-none bg-white/95 backdrop-blur-md px-5 py-3 rounded-xl shadow-2xl border border-slate-200 text-sm z-50 min-w-[160px]"
          ...
          <div className="text-slate-700 mt-1 flex items-center gap-2">
            <span className="font-semibold">{tooltip.count.toLocaleString('pt-BR')}</span> processos
          </div>
        </div>
      )}
    </div>
        ...
        >
          ...
          <div className="flex-1">
            ...
            <div className="h-2.5 bg-neutral-100 rounded-full overflow-hidden">
              <div className="h-full bg-blue-600 transition-all duration-700 ease-out" style={{ width: `${percentage}%` }} />
            </div>
          </div>
        </div>
      );
...
// NOVO: PieChart para Instrumento
        ...
        >
          {data.map((entry, index) => (
            <Cell 
              ...
              stroke={selectedInstrumento === entry.name ? '#2563eb' : 'none'}
              strokeWidth={4}
        ...
        <table className="w-full text-sm text-left text-neutral-700 divide-y divide-neutral-200">
            ...
            {table.getHeaderGroups().map(headerGroup => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map(header => (
                  <th key={header.id} className="px-6 py-4 font-semibold text-neutral-600 uppercase tracking-wider cursor-pointer hover:text-blue-600" onClick={header.column.getToggleSortingHandler()}>
          ...
          <tbody className="divide-y divide-neutral-100">
            {table.getRowModel().rows.length > 0 ? table.getRowModel().rows.map(row => (
              ...
              <tr><td colSpan={columns.length} className="text-center py-12 text-neutral-500 italic">Nenhum registro encontrado</td></tr>
            )}
          </tbody>
        </table>
      </div>
      <div className="flex justify-between items-center p-5 border-t border-neutral-200 bg-neutral-50">
        <button onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()} className="px-5 py-2 bg-white border border-neutral-300 rounded-lg disabled:opacity-40 hover:bg-neutral-100 flex items-center gap-2">
          <ChevronLeft size={16} /> Anterior
        </button>
        <span className="text-sm font-medium text-neutral-600">Página {table.getState().pagination.pageIndex + 1} de {table.getPageCount() || 1}</span>
        <button onClick={() => table.nextPage()} disabled={!table.getCanNextPage()} className="px-5 py-2 bg-white border border-neutral-300 rounded-lg disabled:opacity-40 hover:bg-neutral-100 flex items-center gap-2">
...
// ─────────────────────────────────────────────────────────────────────────────
// DASHBOARD PRINCIPAL – TOTALMENTE REFEITO (Distribuição por INSTRUMENTO)
// ─────────────────────────────────────────────────────────────────────────────
export default function DashboardSneaElis() {
  ...
  const [selectedInstrumento, setSelectedInstrumento] = useState(null);   // ← ALTERADO
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const normalized = allData.map(item => {
        ...
        // SITUAÇÃO (mantida para eficiência)
        ...
        // NOVO: INSTRUMENTO (principal mudança solicitada)
        ...
        return {
          ...
          situacao,           // para eficiência
          instrumento,        // ← NOVO – usado no pie, filtro e tabela
          ano,
  ...
  const filteredData = useMemo(() => 
    data.filter(row => {
      const searchMatch = !searchQuery || Object.values(row).some(v => String(v).toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const metrics = useMemo(() => {
    ...
    const stateAgg = filteredData.reduce((acc, r) => { acc[r.uf] = (acc[r.uf] || 0) + 1; return acc; }, {});
    
    // AGORA POR INSTRUMENTO (principal mudança)
    const instrumentoAgg = filteredData.reduce((acc, r) => { 
      acc[r.instrumento] = (acc[r.instrumento] || 0) + 1; 
      return acc; 
    }, {});

    const yearAgg = filteredData.reduce((acc, r) => { acc[r.ano] = (acc[r.ano] || 0) + r.valor; return acc; }, {});

    const stateRanking = Object.entries(stateAgg).map(([uf, qtd]) => ({ uf, qtd })).sort((a, b) => b.qtd - a.qtd);
    const instrumentoData = Object.entries(instrumentoAgg).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value);
    const yearlyData = Object.entries(yearAgg).map(([ano, valor]) => ({ ano, valor })).sort((a, b) => parseInt(a.ano || 9999) - parseInt(b.ano || 9999));
    ...
    const filterOptions = {
      ...
      instrumentos: [...new Set(data.map(r => r.instrumento).filter(Boolean))].sort()  // ← ALTERADO
    };
  ...
  // TABELA – SUBSTITUÍDO "Situação" por "Instrumento"
  const tableColumns = useMemo(() => [
    ...
    { 
      accessorKey: 'instrumento',   // ← ALTERADO
      header: 'Instrumento',
      cell: ({ getValue }) => {
        const v = getValue();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-500 mt-2">Atualizando KPIs com distribuição por INSTRUMENTO...</p>
        </div>
      </div>
    );
  }
      ...
      <aside className="w-72 bg-slate-950 text-white flex flex-col">
        <div className="p-6 border-b border-slate-800 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">Intelligence PRO • 2026</p>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-y-auto p-8 space-y-8">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="relative flex-1 max-w-2xl">
            ...
            <input type="text" placeholder="Buscar por processo, entidade, UF ou instrumento..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div className="flex items-center gap-4 flex-wrap">
            <FilterSelect label="Ano" value={selectedYear} onChange={setSelectedYear} options={metrics.filterOptions.years} />
            <FilterSelect label="Instrumento" value={selectedInstrumento} onChange={setSelectedInstrumento} options={metrics.filterOptions.instrumentos} /> {/* ← ALTERADO */}
            <button onClick={() => { setSearchQuery(''); setSelectedState(null); setSelectedInstrumento(null); setSelectedYear(null); setTableGlobalFilter(''); }} className="p-3 bg-gray-100 rounded-xl hover:bg-gray-200">
              <RefreshCw size={20} />
            </button>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
          ...
          <KpiCard icon={Target} title="Eficiência" value={metrics.efficiency} color="warning" tooltip="Percentual de processos concluídos" />
          <KpiCard icon={MapPin} title="Estados" value={metrics.stateRanking.length} isActive={!!selectedState} onClick={() => setSelectedState(null)} tooltip="Estados com processos" />
          <KpiCard icon={TrendingUp} title="Evolução" value={metrics.yearlyGrowth} color="info" subtitle={{ text: 'vs ano anterior', color: metrics.growthColor, icon: metrics.growthIcon }} />
        </section>

        {/* MAPA + RANKING */}
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white rounded-2xl shadow-lg p-6 h-[650px] flex flex-col">
            <h3 className="text-xl font-bold mb-4 flex items-center gap-3"><Globe className="text-blue-600" size={24} /> Mapa do Brasil</h3>
            <BrazilMap features={geoFeatures} data={metrics.stateRanking} selectedUf={selectedState} onSelectUf={setSelectedState} hoveredState={hoveredState} onHoverUf={setHoveredState} maxValue={metrics.maxStateValue} />
          </div>

          <div className="bg-white rounded-2xl shadow-lg p-6 h-[650px] flex flex-col">
            <h3 className="text-xl font-bold mb-4 flex items-center gap-3"><BarChart3 className="text-indigo-600" size={24} /> Ranking UF</h3>
            <UfRanking ranking={metrics.stateRanking} selectedUf={selectedState} onSelectUf={setSelectedState} maxValue={metrics.maxStateValue} />
          </div>
        </section>

        {/* GRÁFICOS – AGORA POR INSTRUMENTO */}
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* TABELA – INSTRUMENTO NO LUGAR DE SITUAÇÃO */}
        <section className="bg-white rounded-2xl shadow-lg overflow-hidden">
          <div className="p-6 border-b border-gray-200 flex justify-between items-center">
            ...
            <div className="relative max-w-sm">
              ...
              <input type="text" placeholder="Filtrar tabela..." value={tableGlobalFilter} onChange={e => setTableGlobalFilter(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          <div className="h-[600px]">
            <AdvancedDataTable data={filteredData} columns={tableColumns} globalFilter={tableGlobalFilter} setGlobalFilter={setTableGlobalFilter} />
          </div>
        </section>
      </main>
    </div>
